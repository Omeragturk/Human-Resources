...
namespace Ik_Bitirme.IkApi.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> ForgotPassword(string email)
        {
            ...
            // Kullanıcı kimliğini (ID) gizleyerek ayrı bir token oluştur
            var userIdToken = GenerateTokenForUserId(user.Id);

            // Şifre sıfırlama bağlantısı oluştur
            var callbackUrl = $"{_configuration["AppSettings:ClientUrl"]}ikmvc.azurewebsites.net/User/resetpassword?token={token}&userIdToken={userIdToken}";
        ...
        private string GenerateTokenForUserId(string userId)
        {
            var token = Convert.ToBase64String(Encoding.UTF8.GetBytes(userId));
            return token;
        }
